...
Recently I've been exploring how take actions that actively reduce uncertainty. 
...
In this blog post, I explain exactly why it's hard, and then talk what this means for my research. 
...
Let state be defined as $\mathbf{x}$, controls $\mathbf{u}$.
...
$$
The optimal control sequence can found by solving the following minimization with dynamic programming:
$$
...
# Research
I just described dual control in the context of a dynamical system where uncertainty is a represented as a Gaussian belief $b_k \coloneqq \mathcal{N}(b|\mathbf{\mu}_k, \mathbf{\sigma}_k^2)$ over an unknown parameter $b$ in a linear system.
